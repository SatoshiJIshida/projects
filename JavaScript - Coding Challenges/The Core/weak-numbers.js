 ...
 * Array of two elements: the weakness of the weakest number, and the number of integers in
 * range [1, n] with this weakness.
 * 
 * Tests passed: 12/12.
 * Sample tests: 6/6
 * Hidden tests: 6/6
...
function solution(n) {
    ...
    const findWeakness = ((k) => {
        for (let i = 0; i < collectCounts.length; i++) {
            if ((i < k) && (collectCounts[i] > collectCounts[k])) {
                weakness += 1;
            }
        }
        collectWeakness.push(weakness);
        weakness = 0;
    });
    ...
    let total = 0;
    weakest = Math.max(...collectWeakness);
    for (let i = 0; i < collectWeakness.length; i++) {
        if (collectWeakness[i] === weakest) {
            total += 1;
        }
    }
    result.push(weakest, total);
    return result;
}