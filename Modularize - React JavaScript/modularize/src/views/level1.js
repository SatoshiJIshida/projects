...
import { handleScoreLevel1 } from "../score/scoreSystem";
...
class Level1 extends React.Component {
  ...
  onDragEnd = (result) => {
     ...
     */
    const start = this.state.newData.dropZones[source.droppableId]; // data.dropZones contains droppableIds for source/start.
    const finish = this.state.newData.dropZones[destination.droppableId]; // data.dropZones contains droppableIds for destination/finish.
    ...
    if (start === finish) {
      ...
      const newState = {
        ...
        dropZones: {
          ...this.state.newData.dropZones, // Good-practise to keep this object spread of properties.
    ...
    if (newFinish.itemIds.length === 1) {
      const newState = {
        ...
        dropZones: {
          ...this.state.newData.dropZones, // Keep this object spread of properties.
  ...
  handleChoices = () => {
    // Set correct choices.
    var dropZone1 = this.state.newData.dropZones["dropZone-1"].itemIds.includes(
      "item-1"
    );
    var dropZone2 = this.state.newData.dropZones["dropZone-2"].itemIds.includes(
      "item-2"
    );
    var dropZone3 = this.state.newData.dropZones["dropZone-3"].itemIds.includes(
      "item-3"
    );
    var dropZone4 = this.state.newData.dropZones["dropZone-4"].itemIds.includes(
      "item-4"
    );
    var dropZone5 = this.state.newData.dropZones["dropZone-5"].itemIds.includes(
      "item-5"
    );
    var dropZone6 = this.state.newData.dropZones["dropZone-6"].itemIds.includes(
      "item-6"
    );
    var dropZone7 = this.state.newData.dropZones["dropZone-7"].itemIds.includes(
      "item-7"
    );
    var dropZone8 = this.state.newData.dropZones["dropZone-8"].itemIds.includes(
      "item-8"
    );
    var dropZone9 = this.state.newData.dropZones["dropZone-9"].itemIds.includes(
      "item-9"
    );
    ...
    if (
      ...
      new handleScoreLevel1(); // Call function to calculate the score.
    } else {
      return;
    }
  };
        ...
        <div className="base-bg">
          <div className="level-container">
            <div className="tip-container">
              <div className="h4 fadeIn2">Level 1</div>
              <div className="tip-within">
                <div className="h5 fadeIn2">
                  <div className="fadeRepeat">Tips</div>
                </div>
                <div className="moveRight"><Level1Tips /></div>
              </div>
            </div>

            <DragDropContext key={this.onDragEnd.id} onDragEnd={this.onDragEnd}>
              <div className="dropZone1-bg moveUp1">
                <div className="dropZone-container1">
                  {this.state.newData.dropZoneOrder.map((dropZoneId) => {
                    const dropZone = this.state.newData.dropZones[dropZoneId];
                    const items = dropZone.itemIds.map(
